"""Repeated voltage sweeps for a Keithley 6430 reliability test.

Each repetition follows the same start-to-stop voltage list.  The source is
returned to 0 V after every repetition, and the requested delay is applied
only between repetitions.  All repetitions are saved in one Excel workbook.
"""
...
def make_output_path(
    ...
    repetition_tag = f"rep{repetitions}"
    wait_tag = f"wait{filename_number(wait_time_seconds)}s"
    ...
    base_name = (
        ...
        f"{repetition_tag}_{wait_tag}_{run_tag}.xlsx"
    )
...
def main() -> None:
    ...
    wait_time_seconds = float(
        input("Enter wait time between repetitions (seconds): ")
    )
    ...
    try:
        ...
        # ------------------------------
        # Repeated sweep and averaging
        # ------------------------------
        for repetition in range(1, repetitions + 1):
            print(f"Starting repetition {repetition} of {repetitions}...")

            for point_number, voltage in enumerate(voltages, start=1):
                ...
                measurement_rows.append(
                    {
                        ...
                        "Point in Repetition": point_number,
            ...
            # Every completed sweep ends at 0 V.
            inst.write(":SOUR:VOLT 0")
            time.sleep(ZERO_SETTLE_SECONDS)

            print(f"Completed repetition {repetition} of {repetitions}.")

            # Wait only when another repetition remains.
            if repetition < repetitions:
                print(
                    f"Waiting {wait_time_seconds:g} seconds before "
    ...
    metadata = {
        ...
        "points_per_repetition": len(voltages),
